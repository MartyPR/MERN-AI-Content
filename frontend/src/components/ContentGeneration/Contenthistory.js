...
import { getUserProfileAPI } from "../../services/users/usersAPI";
...
const ContentGenerationHistory = () => {
  ...
  if (isLoading) {
    return <StatusMessage type="loading" message="please wait ...." />;
  } else if (isError) {
    return <StatusMessage type="error" message="something is wrong :c" />;
  }
  console.log(data?.user.contentHistory.length);
  return (
               ...
               return(
                <li className="px-6 py-4 flex items-center justify-between space-x-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500">{new Date(content?.created).toLocaleString()}</p>
                  </div>
